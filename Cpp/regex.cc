...
// Process the paths into rate limit buckets. Creat std::regexs here in the future?
void processBuckets(){
  static bool done = false;
  if(!done){
    std::regex majorParam("\\{(channel\\.id|guild\\.id|webhook\\.id)\\}");
    std::regex anyParam("\\{.+?\\}");
    for(std::string& bucket : buckets){
      bucket = std::regex_replace(bucket, majorParam, "(.+?)");
      bucket = std::regex_replace(bucket, anyParam, ".+?");
    }
    done = true;
  }
}
